  ...
  return (
    <Page title="Autopilot" subtitle="An autonomous prospecting agent: every day it finds fresh leads for your query, enriches and verifies them, scores against your ICP, and can enroll qualified ones into a campaign. Set it and check the pipeline." actions={<button className="btn-primary" onClick={() => setOpen(true)}>New autopilot</button>}>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {aps.map((a) => (
            <div key={a.id} className="card p-4">
              <div className="flex items-start justify-between gap-2"><div><div className="font-semibold">{a.name}</div><div className="text-sm text-slate-600">{a.query.query ?? JSON.stringify(a.query)}</div></div><span className={`badge ${a.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{a.active ? "active" : "paused"}</span></div>
              <div className="mt-2 text-xs text-slate-500">{a.dailyLeads}/day · score ≥ {a.minScore} · {a.requireValidEmail ? "verified email only" : "any email"} · {a.autoEnroll ? "auto-enrolls" : "saves only"} · runs {String(a.runHourUtc).padStart(2, "0")}:00 UTC</div>
              <div className="mt-2 grid grid-cols-4 gap-2 text-center text-xs">{[["runs", a.stats.runs], ["found", a.stats.found], ["saved", a.stats.saved], ["enrolled", a.stats.enrolled]].map(([l, v]) => <div key={String(l)} className="rounded-lg bg-slate-50 p-2"><div className="text-slate-500">{l}</div><div className="text-base font-semibold">{v ?? 0}</div></div>)}</div>
              <div className="mt-2 flex items-center gap-2 text-xs"><span className="text-slate-400">last run {fmtDate(a.lastRunAt)}</span><button className="btn-secondary ml-auto py-1" onClick={() => apiFetch("POST", `/v1/tools/autopilots/${a.id}/run`).then(() => toast("Run queued (takes 1-3 min)"))}>Run now</button><button className="btn-secondary py-1" onClick={() => apiFetch("PATCH", `/v1/tools/autopilots/${a.id}`, { active: !a.active }).then(load)}>{a.active ? "Pause" : "Resume"}</button><button className="text-red-600" onClick={() => apiFetch("DELETE", `/v1/tools/autopilots/${a.id}`).then(load)}>Delete</button></div>
            </div>
          ))}
        </div>
      )}
      <div className="mt-8">
        ...
        {saved.length === 0 ? <div className="text-sm text-slate-500">Save a search from the Find leads page to re-run it daily and get an email when new matches appear.</div> : (
          <div className="card divide-y divide-slate-100">{saved.map((s) => <div key={s.id} className="flex flex-wrap items-center gap-3 p-3 text-sm"><div className="flex-1"><div className="font-medium">{s.name}</div><div className="text-xs text-slate-500">{JSON.stringify(s.query).slice(0, 100)} · {s.alert ? `alerts → ${s.alertEmail}` : "no alerts"} · last run {fmtDate(s.lastRunAt)} · {s.lastNewCount} new</div></div><button className="btn-secondary py-1 text-xs" onClick={() => apiFetch("POST", `/v1/tools/saved-searches/${s.id}/run`).then(() => toast("Queued"))}>Run</button><button className="text-xs text-red-600" onClick={() => apiFetch("DELETE", `/v1/tools/saved-searches/${s.id}`).then(load)}>Delete</button></div>)}</div>
        )}
      </div>
