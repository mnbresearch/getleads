        ...
        <div className="grid gap-3 md:grid-cols-2">
          {aps.map((a) => (
            <div key={a.id} className="card p-4">
              <div className="flex items-start justify-between gap-2"><div><div className="font-semibold">{a.name}</div><div className="text-sm text-ink-300">{a.query.query ?? JSON.stringify(a.query)}</div></div><span className={`badge ${a.active ? "bg-emerald-500/10 text-emerald-300" : "bg-surface/5 text-ink-300"}`}>{a.active ? "active" : "paused"}</span></div>
