...
export function CampaignsPage() {
  ...
  return (
    <Page title="Campaigns" subtitle="AI-personalized sequences with send windows, daily limits, open/click/reply tracking and auto-stop on reply." actions={<><button className="btn-secondary" onClick={() => setAccOpen(true)}>Sender accounts ({accounts.length})</button><button className="btn-primary" onClick={() => setOpen(true)}>New campaign</button></>}>
      {Toast}
      {accounts.length === 0 && <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">Add a sender account first (Resend free tier: 3,000 emails/month, or any SMTP like Brevo/Gmail). <button className="underline" onClick={() => setAccOpen(true)}>Add sender</button></div>}
      {rows.length === 0 ? <Empty title="No campaigns yet" hint="Create a sequence, enroll leads from a list or by ICP score, and start sending." /> : (
        <div className="card overflow-x-auto">
          <table className="w-full min-w-[700px]">
            <thead className="border-b border-slate-200 bg-slate-50"><tr><th className="th">Campaign</th><th className="th">Status</th><th className="th">Contacts</th><th className="th">Sent</th><th className="th">Opened</th><th className="th">Replied</th><th className="th">Created</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {rows.map((c) => (
                <tr key={c.id} className="cursor-pointer hover:bg-slate-50" onClick={() => navigate(`/campaigns/${c.id}`)}>
                  ...
                  <td className="td text-xs text-slate-500">{fmtDate(c.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function StatusBadge({ s }: { s: string }) {
  const m: Record<string, string> = { active: "bg-emerald-50 text-emerald-700", paused: "bg-amber-50 text-amber-700", draft: "bg-slate-100 text-slate-600", completed: "bg-brand-50 text-brand-700", replied: "bg-emerald-50 text-emerald-700", queued: "bg-slate-100 text-slate-600", bounced: "bg-red-50 text-red-700", unsubscribed: "bg-red-50 text-red-700", failed: "bg-red-50 text-red-700", sent: "bg-brand-50 text-brand-700", opened: "bg-emerald-50 text-emerald-700", clicked: "bg-emerald-50 text-emerald-700" };
  return <span className={`badge ${m[s] ?? "bg-slate-100 text-slate-600"}`}>{s}</span>;
}
...
export function CampaignDetail() {
  ...
  return (
    <Page title={c.name} subtitle={`${c.steps?.length ?? 0} steps · ${c.contacts} contacts`} actions={<>
      ...
      {Toast}
      <div className="mb-4 flex items-center gap-2"><StatusBadge s={c.status} /><span className="text-sm text-slate-500">Sends only inside the send window and daily limit. Sequences stop automatically on reply or unsubscribe.</span></div>
      {stats && (
        <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-6">
          {[["Sent", stats.messages.sent], ["Opened", stats.messages.opened, stats.rates.open], ["Clicked", stats.messages.clicked, stats.rates.click], ["Replied", stats.messages.replied, stats.rates.reply], ["Bounced", stats.messages.bounced], ["Failed", stats.messages.failed]].map(([l, v, r]) => (
            <div key={String(l)} className="card p-3"><div className="text-xs uppercase text-slate-500">{l}</div><div className="text-xl font-semibold">{v ?? 0}{r !== undefined && <span className="ml-1 text-xs font-normal text-slate-500">{Math.round(Number(r) * 100)}%</span>}</div></div>
          ))}
        </div>
      )}
      {stats?.variants && stats.variants.length > 1 && <div className="card mb-4 p-3 text-sm"><div className="mb-1 font-medium">A/B results</div><div className="flex flex-wrap gap-3">{stats.variants.map((v, i) => <div key={i} className="rounded-lg bg-slate-50 px-3 py-2 text-xs">Step {(c.steps ?? []).findIndex((st) => (st as unknown as { id: string }).id === v.stepId) + 1 || "?"} · Variant {String.fromCharCode(65 + v.variant)}: {v.sent} sent · {v.sent ? Math.round((v.opened / v.sent) * 100) : 0}% open · {v.sent ? Math.round((v.replied / v.sent) * 100) : 0}% reply</div>)}</div></div>}
      <div className="mb-3 flex gap-2 border-b border-slate-200">{(["contacts", "messages"] as const).map((t) => <button key={t} className={`px-3 py-2 text-sm capitalize ${tab === t ? "border-b-2 border-brand-600 font-medium text-brand-700" : "text-slate-500"}`} onClick={() => setTab(t)}>{t}</button>)}</div>
      {tab === "contacts" ? (
        <div className="card overflow-x-auto">
          <table className="w-full min-w-[700px]">
            <thead className="border-b border-slate-200 bg-slate-50"><tr><th className="th">Lead</th><th className="th">Email</th><th className="th">Status</th><th className="th">Step</th><th className="th">Next send</th><th className="th"></th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {contacts.map((x) => (
                <tr key={x.id}>
                  <td className="td"><div className="font-medium">{x.lead.fullName}</div><div className="text-xs text-slate-500">{x.lead.title} · {x.lead.company?.name}</div></td>
                  ...
                  <td className="td text-xs text-slate-500">{fmtDate(x.nextSendAt)}</td>
              ...
              {contacts.length === 0 && <tr><td colSpan={6} className="td py-8 text-center text-slate-500">No contacts enrolled. Use "Enroll leads".</td></tr>}
            </tbody>
          </table>
        </div>
      ) : (
        <div className="card divide-y divide-slate-100">
          {messages.map((m) => (
            <details key={m.id} className="p-3">
              <summary className="flex cursor-pointer flex-wrap items-center gap-2 text-sm"><StatusBadge s={m.status} /><span className="font-medium">{m.subject}</span><span className="text-slate-500">{m.direction === "inbound" ? "from" : "to"} {m.toEmail}</span><span className="ml-auto text-xs text-slate-400">{fmtDate(m.sentAt)}{m.openedAt && " · opened"}{m.repliedAt && " · replied"}</span></summary>
              <pre className="mt-2 whitespace-pre-wrap font-sans text-sm text-slate-700">{m.bodyText}</pre>
            </details>
          ))}
  ...
  return (
    <Modal open={open} onClose={onClose} title="Enroll leads">
      <div className="space-y-3 text-sm">
        ...
        <p className="text-xs text-slate-500">Only leads with a non-invalid email are enrolled. Suppressed/unsubscribed addresses are always skipped.</p>
