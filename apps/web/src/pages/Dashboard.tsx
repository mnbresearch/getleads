...
export function Dashboard() {
  ...
  if (err) return <Page title="Overview"><div className="text-red-300">{err}</div></Page>;
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <div className="card p-4 lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            ...
            <div className="flex gap-3 text-xs text-ink-400"><span><i className="mr-1 inline-block h-2 w-2 rounded-sm bg-brand-500/100" />leads</span><span><i className="mr-1 inline-block h-2 w-2 rounded-sm bg-emerald-500/100" />sent</span><span><i className="mr-1 inline-block h-2 w-2 rounded-sm bg-amber-500/100" />replied</span></div>
          </div>
          <div className="flex h-40 items-end gap-[3px]">
            {d.daily.map((x) => (
              <div key={x.day} className="group relative flex flex-1 items-end gap-px" title={`${x.day}: ${x.leads} leads, ${x.sent} sent, ${x.replied} replied`}>
                <div className="flex-1 rounded-t bg-brand-500/100/80" style={{ height: `${(x.leads / max) * 100}%` }} />
                <div className="flex-1 rounded-t bg-emerald-500/100/80" style={{ height: `${(x.sent / max) * 100}%` }} />
                <div className="flex-1 rounded-t bg-amber-500/100/80" style={{ height: `${(x.replied / max) * 100}%` }} />
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {Object.entries(d.usage.usage).map(([k, v]) => (
              <div key={k}>
                ...
                <div className="mt-1 h-1.5 rounded-full bg-surface/5"><div className={`h-1.5 rounded-full ${v.limit && v.used / v.limit > 0.9 ? "bg-red-500/100" : "bg-brand-500/100"}`} style={{ width: `${v.limit ? Math.min(100, (v.used / v.limit) * 100) : 0}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(d.emailStatus).map(([s, n]) => (
              <div key={s} className="rounded-lg border border-white/10 px-3 py-2 text-sm"><span className="text-ink-400">{s.replace("_", " ")}</span> <span className="ml-2 font-semibold">{fmtNum(n)}</span></div>
            ))}
