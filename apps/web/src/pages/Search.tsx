...
export function SearchPage() {
  ...
  return (
    <Page title="Find leads" subtitle="Describe who you want. GetLeads searches the open web + LinkedIn, enriches companies, finds and verifies emails, and scores fit.">
      {Toast}
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="card p-5 text-sm text-slate-600">
          <div className="mb-2 font-medium text-slate-900">How it works</div>
      ...
      <div className="card mt-6 overflow-x-auto">
        <table className="w-full">
          <thead className="border-b border-slate-200 bg-slate-50"><tr><th className="th">When</th><th className="th">Query</th><th className="th">Status</th><th className="th">Results</th><th className="th"></th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            {searches.map((s) => (
              <tr key={s.id}>
                <td className="td whitespace-nowrap text-slate-500">{fmtDate(s.createdAt)}</td>
            ...
            {searches.length === 0 && <tr><td className="td py-8 text-center text-slate-500" colSpan={5}>No searches yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </Page>
