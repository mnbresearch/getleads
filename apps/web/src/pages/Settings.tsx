...
export function SettingsPage() {
  ...
  return (
    <Page title="Settings">
      <div className="mb-4 flex gap-1 border-b border-slate-200">{tabs.map(([p, l]) => <NavLink key={p} to={`/settings/${p}`} end className={({ isActive }) => `px-3 py-2 text-sm ${isActive ? "border-b-2 border-brand-600 font-medium text-brand-700" : "text-slate-500"}`}>{l}</NavLink>)}</div>
      <Routes>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-5">
        ...
        {fresh && <div className="mb-3 rounded-lg bg-slate-900 p-3 text-xs text-emerald-300"><div className="mb-1 text-slate-300">Copy now - shown once:</div><code className="break-all">{fresh}</code></div>}
      ...
      <div className="card p-5 text-sm">
        ...
        <p className="text-slate-600">REST: send <code>x-api-key</code>. OpenAPI spec at <a className="text-brand-600" href={`${API_URL}/openapi.json`} target="_blank" rel="noreferrer">{API_URL}/openapi.json</a>, interactive docs at <a className="text-brand-600" href={`${API_URL}/docs`} target="_blank" rel="noreferrer">/docs</a>.</p>
        <p className="mt-2 text-slate-600">MCP (Claude Desktop, Claude Code, Cursor):</p>
        <pre className="mt-1 overflow-x-auto rounded-lg bg-slate-900 p-3 text-xs text-emerald-200">{`{ "mcpServers": { "getleads": { "command": "npx", "args": ["-y", "@getleads/mcp"],
    "env": { "GETLEADS_API_KEY": "gl_live_...", "GETLEADS_API_URL": "${API_URL}" } } } }`}</pre>
      </div>
    </div>
  );
}
...
const PROVIDER_FIELDS: Record<string, { label: string; fields: [string, string][]; help: string }> = {
  ...
  cortex: { label: "Cortex (your automation platform)", fields: [["url", "Cortex webhook / ingest URL"], ["authHeader", "Authorization header value (optional)"]], help: "GetLeads POSTs {source, lead, company} JSON to this URL." },
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="card p-5">
        ...
        <p className="mb-3 text-xs text-slate-500">{p.help}</p>
      ...
      <div className="card p-5">
        ...
        <ul className="divide-y divide-slate-100 text-sm">{list.map((i) => <li key={i.provider} className="flex items-center justify-between py-2"><div><div className="font-medium">{PROVIDER_FIELDS[i.provider]?.label ?? i.provider}</div><div className="text-xs text-slate-500">last sync {fmtDate(i.lastSyncAt)}</div></div><button className="text-red-600" onClick={() => apiFetch("DELETE", `/v1/integrations/${i.provider}`).then(load)}>Disconnect</button></li>)}{list.length === 0 && <li className="py-2 text-slate-500">Nothing connected yet.</li>}</ul>
        <p className="mt-3 text-xs text-slate-500">Push leads from the Leads page (select → sync) or via <code>POST /v1/integrations/{"{provider}"}/sync</code>.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="card p-5"><div className="mb-3 font-medium">Current plan: <span className="capitalize">{u.plan}</span> · {u.period}</div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{Object.entries(u.usage).map(([k, v]) => <div key={k} className="rounded-lg border border-slate-200 p-3"><div className="text-xs capitalize text-slate-500">{k.replace(/([A-Z])/g, " $1")}</div><div className="text-lg font-semibold">{v.used.toLocaleString()} <span className="text-xs font-normal text-slate-400">/ {v.limit.toLocaleString()}</span></div></div>)}</div>
        {plans.pilotMode && <p className="mt-3 text-sm text-emerald-700">Pilot mode: everything is free during the pilot. Limits reset monthly.</p>}
      </div>
      <div className="grid gap-3 md:grid-cols-4">{plans.plans.map((p) => <div key={p.id} className={`card p-4 ${p.id === u.plan ? "ring-2 ring-brand-500" : ""}`}><div className="font-semibold">{p.name}</div><div className="text-2xl font-semibold">${p.priceUsd}<span className="text-sm font-normal text-slate-500">/mo</span></div><ul className="mt-2 space-y-0.5 text-xs text-slate-600"><li>{Number(p.limits.leadsPerMonth).toLocaleString()} leads/mo</li><li>{Number(p.limits.verificationsPerMonth).toLocaleString()} verifications</li><li>{Number(p.limits.aiMessagesPerMonth).toLocaleString()} AI messages</li><li>{Number(p.limits.emailsPerMonth).toLocaleString()} emails</li><li>{p.limits.campaigns as number} campaigns</li></ul>{plans.stripeEnabled && p.priceUsd > 0 && p.id !== u.plan && <button className="btn-primary mt-3 w-full justify-center" onClick={() => apiFetch<{ url: string }>("POST", "/v1/billing/checkout", { plan: p.id }).then((r) => (location.href = r.url))}>Upgrade</button>}</div>)}</div>
    </div>
  );
}
...
function Team() {
  ...
  return (
    <div className="space-y-4">
      {Toast}
      <div className="card p-5">
        <div className="mb-3 flex items-center justify-between"><div className="font-medium">Members <span className="text-sm text-slate-500">{d.seats.used} / {d.seats.limit} seats</span></div></div>
        <ul className="divide-y divide-slate-100 text-sm">{d.members.map((m) => <li key={m.id} className="flex items-center justify-between py-2"><div>{m.name || m.email} <span className="text-slate-500">{m.email}</span> <span className="badge ml-2 bg-slate-100 text-slate-600">{m.role}</span></div><span className="text-xs text-slate-400">last login {fmtDate(m.lastLoginAt)}</span></li>)}</ul>
        ...
        {link && <div className="mt-2 text-xs text-slate-500">Invite link (also emailed if a mail provider is configured): <code className="break-all">{link}</code></div>}
        {d.invites.length > 0 && <div className="mt-3 text-xs text-slate-500">Pending: {d.invites.map((i) => i.email).join(", ")}</div>}
      </div>
    </div>
  );
}
