...
const TypeBadge = ({ t }: { t: string }) => <span className={`badge ${{ funding: "bg-emerald-50 text-emerald-700", acquisition: "bg-purple-50 text-purple-700", hiring: "bg-brand-50 text-brand-700", leadership: "bg-amber-50 text-amber-700" }[t] ?? "bg-slate-100 text-slate-600"}`}>{t}</span>;
...
export function SignalsPage() {
  ...
  return (
    <Page title="Intent signals" subtitle="Companies that just raised money, got acquired, hired a new leader or are hiring fast are 3-5x more likely to buy. Subscribe and GetLeads turns signals into decision-maker leads automatically." actions={<><button className="btn-secondary" onClick={scan} disabled={scanning}>{scanning ? "Scanning news…" : "Scan now"}</button><button className="btn-secondary" onClick={() => setMonOpen(true)}>New monitor</button><button className="btn-primary" onClick={() => setSubOpen(true)}>New subscription</button></>}>
      {Toast}
      <div className="mb-3 flex gap-2 border-b border-slate-200">{(["feed", "subscriptions", "monitors"] as const).map((t) => <button key={t} className={`px-3 py-2 text-sm capitalize ${tab === t ? "border-b-2 border-brand-600 font-medium text-brand-700" : "text-slate-500"}`} onClick={() => setTab(t)}>{t}{t === "subscriptions" ? ` (${subs.length})` : t === "monitors" ? ` (${mons.length})` : ""}</button>)}</div>
            ...
            {signals.map((s) => (
              <div key={s.id} className="flex flex-wrap items-start gap-3 p-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-medium">{s.companyName ?? "Unknown company"} {s.amountUsd ? <span className="ml-1 text-emerald-700">{money(s.amountUsd)}</span> : null} {s.round && <span className="ml-1 text-xs text-slate-500">{s.round}</span>}</div>
                  <a className="text-sm text-slate-600 hover:underline" href={s.url} target="_blank" rel="noreferrer">{s.title}</a>
                  <div className="text-xs text-slate-400">{s.source} · {fmtDate(s.occurredAt ?? s.createdAt)} {s.match && <span className="ml-2 badge bg-brand-50 text-brand-700">matched{s.match.leadsCreated ? ` · ${s.match.leadsCreated} leads` : ""}</span>}</div>
                </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {subs.map((s) => (
            <div key={s.id} className="card p-4">
              ...
              <div className="mt-2 text-xs text-slate-500">Keywords: {[...s.keywords, ...s.industries, ...s.locations].join(", ") || "any"} · Targets: {s.targetTitles.join(", ")}</div>
              <div className="mt-1 text-xs text-slate-500">{s.autoCreateLeads ? "Auto-creates leads" : "Match only"} · matched {s.stats.matched ?? 0} · leads {s.stats.leadsCreated ?? 0} · last run {fmtDate(s.lastRunAt)}</div>
            </div>
          ))}
        </div>
      ))}
      ...
      <Modal open={!!results} onClose={() => setResults(null)} title={results?.m.name ?? ""} wide>
        <div className="max-h-[60vh] divide-y divide-slate-100 overflow-y-auto text-sm">
          {results?.rows.map((r) => <div key={r.id} className="py-2"><span className="badge mr-2 bg-slate-100 text-slate-600">{r.kind}</span>{r.url && !r.url.startsWith("job:") ? <a className="hover:underline" href={r.url} target="_blank" rel="noreferrer">{r.title}</a> : r.title}{r.leadId && <span className="ml-2 badge bg-emerald-50 text-emerald-700">lead</span>}<div className="text-xs text-slate-500">{r.snippet}</div></div>)}
          {results?.rows.length === 0 && <div className="py-6 text-center text-slate-500">No results yet.</div>}
        </div>
      </Modal>
    </Page>
  );
