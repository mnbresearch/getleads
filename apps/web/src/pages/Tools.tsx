    ...
    <Page title="Tools" subtitle="One-off enrichment and verification tools. Everything here is also available in the API and MCP server.">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="card p-4 lg:col-span-1">
          <div className="space-y-1">{(Object.keys(meta) as (typeof tool)[]).map((k) => <button key={k} className={`block w-full rounded-lg px-3 py-2 text-left text-sm ${tool === k ? "bg-brand-50 font-medium text-brand-700" : "hover:bg-slate-50"}`} onClick={() => { setTool(k); setOut(null); }}>{meta[k].label}</button>)}</div>
        </div>
        <div className="card space-y-3 p-4 lg:col-span-2">
          ...
          {out && (Array.isArray(out) ? (
            <div className="overflow-x-auto">
              ...
              {out.length === 0 && <div className="py-4 text-sm text-slate-500">No results (keyless search engines are rate-limited from cloud IPs; add a Brave/Google key or Apollo/Hunter for consistent results).</div>}
            </div>
          ) : <pre className="max-h-96 overflow-auto rounded-lg bg-slate-900 p-3 text-xs text-emerald-200">{JSON.stringify(out, null, 2)}</pre>)}
        </div>
      </div>
    </Page>
  );
